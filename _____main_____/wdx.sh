...
echo -e "   '----------------------------------'"
echo -e "     \e[1;32m1)\e[0m sudah daftar ip"
echo -e "     \e[1;32m2)\e[0m belum daftar ip"
echo -e "   ------------------------------------"
read -p "   select numbers 1-2 : " ipku
echo ""
if [[ $ipku == "1" ]]; then
echo -e ""
# install script
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/LunaticTunnel/permission/_/ipme | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/LunaticTunnel/permission/_/ipme | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/LunaticTunnel/permission/_/ipme | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/LunaticTunnel/permission/_/ipme | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/LunaticTunnel/permission/_/ipme | grep $ipsaya | awk '{print $3}')
...
wget raw.githubusercontent.com/LunaticTunnel/XDG/_/___files___/limit.sh && chmod +x limit.sh && ./limit.sh
